...
$dirToMount = "${localRepoDir}\Cheat_Sheet_Repository\SQL\Sample Databases\Microsoft SQL Server\Personal Database\Data"
$mountPoint = "/var/opt/mssql/data"
$hostPort = "4433"
$containerName = "personal_db"
...
docker run -e "ACCEPT_EULA=Y" -e "MSSQL_SA_PASSWORD=P@ssw0rd" -p "${hostPort}:1433" --name ${containerName} -d mcr.microsoft.com/mssql/server:2019-latest
for ($i = 0; $i -lt 60; $i++) {
